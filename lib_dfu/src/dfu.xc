                                                   ...
                                                   int32_t &?block_num)
{
  ...
  if (request == DFU_GETSTATUS && !isnull(block) && block_size_bytes == DFU_GET_STATUS_PAYLOAD_SIZE_BYTES) {
    ...
    memcpy(&block[DFU_GETSTATUS_POLL_TIMEOUT_INDEX], &ret_poll_timeout_msec, DFU_GETSTATUS_POLL_TIMEOUT_BYTES);
