...
int main(void)
{
  ...
  unsigned char template[32] = {
    ...
    0x12, 0x34, 0x00, 0xCD, 0x00, 0xAB, 0x01, 0x10,
  ...
  memcpy(data, template, 32);
  data[21] = 0x00;
  ret = verify_dfu_suffix(data, 32, 0xFFFF, 0xCD, &suffix_length, msg);
  printf("%d\n", ret);
  assert(ret == 0);
  ...
  memcpy(data, template, 32);
  data[19] = 0x00;
  ret = verify_dfu_suffix(data, 32, 0xAB, 0xFFFF, &suffix_length, msg);
  printf("%d\n", ret);
  assert(ret == 0);
